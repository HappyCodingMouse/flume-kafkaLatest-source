 ...
 */
package cxf.flume.source.kafka;
...
import static cxf.flume.source.kafka.NewKafkaSourceConstants.*;
 ...
 * <p>
 */
public class NewKafkaSource extends AbstractPollableSource
        implements Configurable {
  private static final Logger log = LoggerFactory.getLogger(NewKafkaSource.class);
...
//  private boolean migrateZookeeperOffsets = DEFAULT_MIGRATE_ZOOKEEPER_OFFSETS;
  ...
  public abstract class Subscriber<T> {
    public abstract void subscribe(KafkaConsumer<?, ?> consumer, NewSourceRebalanceListener listener);
  ...
  private class TopicListSubscriber extends Subscriber<List<String>> {
    ...
    @Override
    public void subscribe(KafkaConsumer<?, ?> consumer, NewSourceRebalanceListener listener) {
  ...
  protected Status doProcess() throws EventDeliveryException {
    ...
    try {
      ...
      while (eventList.size() < batchUpperLimit &&
        ...
        if (!headers.containsKey(NewKafkaSourceConstants.TIMESTAMP_HEADER)) {
          headers.put(NewKafkaSourceConstants.TIMESTAMP_HEADER,
        ...
        if (!headers.containsKey(NewKafkaSourceConstants.PARTITION_HEADER)) {
          headers.put(NewKafkaSourceConstants.PARTITION_HEADER,
              String.valueOf(message.partition()));
        }

        if (kafkaKey != null) {
          headers.put(NewKafkaSourceConstants.KEY_HEADER, kafkaKey);
        }
      ...
      log.error("NewKafkaSource EXCEPTION, {}", e);
      return Status.BACKOFF;
    }
  }
  ...
  protected void doConfigure(Context context) throws FlumeException {
    ...
    String topicProperty = context.getString(NewKafkaSourceConstants.TOPICS_REGEX);
    ...
    } else if ((topicProperty = context.getString(NewKafkaSourceConstants.TOPICS)) != null &&
    ...
    batchUpperLimit = context.getInteger(NewKafkaSourceConstants.BATCH_SIZE,
                                         NewKafkaSourceConstants.DEFAULT_BATCH_SIZE);
    maxBatchDurationMillis = context.getInteger(NewKafkaSourceConstants.BATCH_DURATION_MS,
                                                NewKafkaSourceConstants.DEFAULT_BATCH_DURATION);

    useAvroEventFormat = context.getBoolean(NewKafkaSourceConstants.AVRO_EVENT,
                                            NewKafkaSourceConstants.DEFAULT_AVRO_EVENT);

    if (log.isDebugEnabled()) {
      log.debug(NewKafkaSourceConstants.AVRO_EVENT + " set to: {}", useAvroEventFormat);
    }
...
//        DEFAULT_MIGRATE_ZOOKEEPER_OFFSETS);
    ...
    bootstrapServers = context.getString(NewKafkaSourceConstants.BOOTSTRAP_SERVERS);
...
//            NewKafkaSourceConstants.ZOOKEEPER_CONNECT_FLUME_KEY,
//            NewKafkaSourceConstants.BOOTSTRAP_SERVERS);
//
//        // Lookup configured security protocol, just in case its not default
//        String securityProtocolStr =
//            context.getSubProperties(NewKafkaSourceConstants.KAFKA_CONSUMER_PREFIX)
...
//      }
    ...
    setTopicHeader = context.getBoolean(NewKafkaSourceConstants.SET_TOPIC_HEADER,
                                        NewKafkaSourceConstants.DEFAULT_SET_TOPIC_HEADER);

    topicHeader = context.getString(NewKafkaSourceConstants.TOPIC_HEADER,
                                    NewKafkaSourceConstants.DEFAULT_TOPIC_HEADER);
  ...
  private void translateOldProperties(Context ctx) {
    // topic
    String topic = context.getString(NewKafkaSourceConstants.TOPIC);
    if (topic != null && !topic.isEmpty()) {
      subscriber = new TopicListSubscriber(topic);
      log.warn("{} is deprecated. Please use the parameter {}",
              NewKafkaSourceConstants.TOPIC, NewKafkaSourceConstants.TOPICS);
    }

    // old groupId
    groupId = ctx.getString(NewKafkaSourceConstants.OLD_GROUP_ID);
    if (groupId != null && !groupId.isEmpty()) {
      log.warn("{} is deprecated. Please use the parameter {}",
              NewKafkaSourceConstants.OLD_GROUP_ID,
              NewKafkaSourceConstants.KAFKA_CONSUMER_PREFIX + ConsumerConfig.GROUP_ID_CONFIG);
    }
  }
  ...
  private void setConsumerProps(Context ctx) {
    ...
    kafkaProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG,
                   NewKafkaSourceConstants.DEFAULT_KEY_DESERIALIZER);
    kafkaProps.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG,
                   NewKafkaSourceConstants.DEFAULT_VALUE_DESERIALIZER);
    //Defaults overridden based on config
    kafkaProps.putAll(ctx.getSubProperties(NewKafkaSourceConstants.KAFKA_CONSUMER_PREFIX));
    ...
    kafkaProps.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG,
                   NewKafkaSourceConstants.DEFAULT_AUTO_COMMIT);
  }
...
//    }
    ...
    subscriber.subscribe(consumer, new NewSourceRebalanceListener(rebalanceFlag));
...
class NewSourceRebalanceListener implements ConsumerRebalanceListener {
  private static final Logger log = LoggerFactory.getLogger(NewSourceRebalanceListener.class);
  private AtomicBoolean rebalanceFlag;

  public NewSourceRebalanceListener(AtomicBoolean rebalanceFlag) {
